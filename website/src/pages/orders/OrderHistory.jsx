  ...
  return (
    <div className="pt-[40px] pb-[100px] px-5 min-h-screen">
      ...
      <div className="w-full max-w-[1000px] mx-auto">
        <div className="flex flex-wrap gap-2 justify-center p-2 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 mb-10">
          {TABS.map(tab => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)} 
              className={`py-2 px-6 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${
                activeTab === tab 
        ...
        <div className="flex flex-col gap-4">
          {filteredItems.length === 0 ? (
            <div className="p-16 text-center bg-white/5 rounded-[24px] border border-dashed border-white/20">
              <p className="text-white/60 font-black text-[14px] uppercase tracking-widest">No Products Found</p>
            </div>
          ) : (
            filteredItems.map((item, index) => {
              ...
              return (
                <div 
                  ...
                  className="bg-white rounded-2xl p-4 flex flex-col md:flex-row items-start md:items-center gap-5 cursor-pointer hover:shadow-lg transition-all border border-slate-200"
                >
                  <div className="w-24 h-24 bg-slate-50 rounded-xl border border-slate-100 overflow-hidden shrink-0">
                    <img src={item.image} alt={item.product_name} className="w-full h-full object-cover" />
                  </div>
                  
                  <div className="flex-1 flex flex-col gap-1 w-full">
                    <h4 className="text-[17px] font-black text-slate-900">{item.product_name}</h4>
                    <p className="text-[13px] font-bold text-slate-500">{item.created_at}</p>
                    <span className="text-[14px] font-black text-slate-800 mt-1">Qty: {item.quantity}</span>
                  </div>

                  <div className="flex flex-col items-start md:items-end gap-2 w-full md:w-auto shrink-0">
                    <p className="text-[18px] font-black text-slate-900">₹{Number(item.price).toLocaleString('en-IN')}</p>
                    <span className={`px-3 py-1 rounded-md text-[12px] font-black uppercase border ${getStatusStyle(item.status)}`}>
                      {getDisplayStatus(item.status)}
                    </span>
                  </div>

                  {showItemReturn && (
                    <div className="w-full md:w-auto shrink-0 mt-2 md:mt-0 flex justify-end">
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                        className="px-5 py-2.5 bg-slate-900 text-white font-black text-[11px] uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-colors whitespace-nowrap"
                      >
