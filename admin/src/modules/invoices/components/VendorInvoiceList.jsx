...
const VendorInvoiceList = ({
    ...
    showToast,
    mode = 'summary' // 'summary' for vendors, 'detail' for individual invoices
}) => {
            ...
            <div className="inv-toolbar">
                <div className="inv-filters">
                    <div className="inv-search-wrap">
                        ...
                        <input
                            type="text"
                            placeholder={mode === 'summary' ? "Search vendor name or ID..." : "Search invoice or order ID..."}
            ...
            <div className="inv-table-scroll">
                <table className="inv-table">
                    <thead className="inv-thead">
                        {mode === 'summary' ? (
                            <tr>
                                ...
                                <th className="inv-th">VENDOR ID</th>
                                <th className="inv-th">BUSINESS NAME</th>
                                <th className="inv-th">VENDOR NAME</th>
                                <th className="inv-th">VENDOR DETAILS</th>
                                <th className="inv-th-center">TOTAL INVOICES</th>
                                <th className="inv-th">TOTAL AMOUNT</th>
                                <th className="inv-th">PENDING AMOUNT</th>
                                <th className="inv-th">LAST INVOICE</th>
                    ...
                    <tbody className="inv-tbody">
                        {invoices.length === 0 ? (
                            <tr>
                                <td colSpan={mode === 'summary' ? 11 : 9} className="inv-empty-cell">
                                    <div className="inv-empty-title">No {mode === 'summary' ? 'Vendors' : 'Invoices'} Found</div>
                                    <p className="inv-empty-sub">Try adjusting your filters or search terms.</p>
                                </td>
                            </tr>
                        ) : (
                            invoices.map((item) => {
                                const rowId = mode === 'summary' ? (item.vendorId) : item.id;
                                return (
                                    ...
                                    {mode === 'summary' ? (
                                        <>
                                            ...
                                            <td className="inv-td">
                                                <span className="inv-badge-orange" style={{ fontSize: '12px' }}>{item.vendorId}</span>
                                            </td>
                                            <td className="inv-td">
                                                <div style={{ fontWeight: 700, color: '#1e293b' }}>{item.vendorName}</div>
                                            </td>
                                            <td className="inv-td">
                                                <div style={{ fontWeight: 600, color: '#475569' }}>{item.vendorOwnerName}</div>
                                            </td>
                                            <td className="inv-td">
                                                <div className="inv-val-phone" style={{ fontSize: '12px' }}>{item.vendorPhone}</div>
                                                <div style={{ fontSize: '11px', color: '#94a3b8' }}>{item.vendorEmail}</div>
                                            </td>
                                            <td className="inv-td-center">
                                        ...
                                        <>
                                            ...
                                            <td className="inv-td-right">
                                                <div className="inv-actions">
                                                    <button
                                                        onClick={() => onView(item)} // Call onView function
