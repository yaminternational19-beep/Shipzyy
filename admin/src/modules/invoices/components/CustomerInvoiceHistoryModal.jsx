...
const CustomerInvoiceHistoryModal = ({ customerId, onClose }) => {
                ...
                <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        <div style={{ width: '50px', height: '50px', borderRadius: '50%', overflow: 'hidden' }}>
                            <img src={customerInfo.avatar} alt="Profile" style={{ width: '100%', height: '100%' }} />
                        </div>
                        <div>
                            <h2 style={{ margin: 0, fontSize: '1.5rem', color: 'var(--text-primary)', fontWeight: 700 }}>{customerInfo.name || 'Anonymous Customer'}</h2>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                                <span className="cust-id-badge" style={{ margin: 0 }}>{customerId}</span>
                                <span style={{ color: '#64748b', fontSize: '0.85rem' }}>Invoice History</span>
                            </div>
                        </div>
                    </div>
